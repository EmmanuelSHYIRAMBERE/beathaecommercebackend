...
export const getAllBookingOfUser = catchAsyncError(async (req, res, next) => {
  ...
  if (!bookings || bookings.length === 0) {
    return next(
      new errorHandler(`You do not have any pending reserved slot .`, 404)
    );
  }

  const userPendingReservedSlot = [];
  ...
  res.status(200).json(userPendingReservedSlot);
});
